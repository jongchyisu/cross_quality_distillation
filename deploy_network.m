function net = deploy_network(net)
%CNN_IMAGENET_DEPLOY  Deploy a CNN

isDag = isa(net, 'dagnn.DagNN') ;
if isDag
  % lossLayers = dagFindLayersOfType(net, 'dagnn.Loss');
  lossLayers = dagFindPredictionNamesWithOutput(net, 'objective');
  
  if numel(lossLayers)>1
    for i = 1:numel(lossLayers)
      net.addLayer(['prob' num2str(i)], dagnn.SoftMax(), lossLayers{i}, ['prob' num2str(i)], {}) ;
    end
  else
    net.addLayer('prob', dagnn.SoftMax(), 'prediction', 'prob', {}) ;
  end
  
  dagRemoveLayersOfType(net, 'Loss_Hierarchy') ;
  dagRemoveLayersOfType(net, 'Loss_Mask_Class') ;
...
if ~isDag
  for l = simpleFindLayersOfTdagFindLayersWithOutputype(net, 'conv')
...
% -------------------------------------------------------------------------
function names = dagFindPredictionNamesWithOutput(net, outVarName)
% -------------------------------------------------------------------------
names = {} ;
for l = 1:numel(net.layers)
  if any(strncmp(net.layers(l).outputs, outVarName, length(outVarName)))
    names{1,end+1} = net.layers(l).inputs{1} ;
  end
end
