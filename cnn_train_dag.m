...
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
    ...
    inputs = state.getBatch(state.imdb, batch, state.epoch) ;
    ...
    if opts.prefetch
      ...
      state.getBatch(state.imdb, nextBatch, state.epoch) ;
    end
    ...
    if strcmp(mode, 'train')
      ...
      if opts.useDistill
          opts.derOutputs = cat(2, opts.derOutputs, 'distilled_loss', opts.lambda);
      end
...
for p=1:numel(net.params)
  if ~isempty(net.params(p).der) % added for multi-dataset
...
% -------------------------------------------------------------------------
for i=1:numel(net.params)
  if ~isempty(net.params(i).der) % added for multi-dataset
...
sel = find(cellfun(@(x)  isa(x,'Loss_Hierarchy'), {net.layers.block})) ;
for i = 1:numel(sel)
  stats.(net.layers(sel(i)).outputs{1}) = sum(net.layers(sel(i)).block.average) ;
end
sel = find(cellfun(@(x)  isa(x,'Loss_Mask_Class'), {net.layers.block})) ;
for i = 1:numel(sel)
  stats.(net.layers(sel(i)).outputs{1}) = sum(net.layers(sel(i)).block.average) ;
end
